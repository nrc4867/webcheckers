...
public class PostSignOutRoute implements Route {
    ...
    public Object handle(Request request, Response response) throws Exception {
        ...
        if (request.attribute(NavBar.PLAYER_SIGNIN_KEY) != null) { 
            if(!(request.attribute(NavBar.PLAYER_SIGNIN_KEY) instanceof Player)) return null;
            Player player = (Player) request.attribute(NavBar.PLAYER_SIGNIN_KEY);
            //TODO: lobby.freeName(player); // remove the player from the lobby
            request.attribute(NavBar.PLAYER_SIGNIN_KEY, null); // remove sign-in from session
        }
